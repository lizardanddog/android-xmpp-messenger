...
public class CameraFragment extends Fragment {
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                int rotatedPreviewWidth = width, rotatedPreviewHeight = height, maxPreviewWidth = displaySize.x, maxPreviewHeight = displaySize.y;
                if (swappedDimensions) {
                    rotatedPreviewWidth = height; rotatedPreviewHeight = width; maxPreviewWidth = displaySize.y; maxPreviewHeight = displaySize.x;
                }
    ...
    private void configureTransform(int viewWidth, int viewHeight) {
        ...
        RectF viewRect = new RectF(0, 0, viewWidth, viewHeight), bufferRect = new RectF(0, 0, mPreviewSize.getHeight(), mPreviewSize.getWidth());
    ...
    private static Size chooseOptimalSize(Size[] choices, int textureViewWidth, int textureViewHeight, int maxWidth, int maxHeight, Size aspectRatio) {
        List<Size> bigEnough = new ArrayList<>(), notBigEnough = new ArrayList<>();
    ...
    private void showToast(final String text) {
        final Activity activity = getActivity();
        if (activity != null) activity.runOnUiThread(() -> Toast.makeText(activity, text, Toast.LENGTH_SHORT).show());
    }
    ...
    public static class ErrorDialog extends DialogFragment {
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            final Activity activity = getActivity();
            return new AlertDialog.Builder(activity).setMessage(getArguments().getString(ARG_MESSAGE)).setPositiveButton(android.R.string.ok, (dialogInterface, i) -> activity.finish()).create();
        }
    }
    ...
    public static class ConfirmationDialog extends DialogFragment {
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            return new AlertDialog.Builder(getActivity()).setMessage(R.string.app_name).setPositiveButton(android.R.string.ok, (dialog, which) -> parent.requestPermissions(new String[]{Manifest.permission.CAMERA}, REQUEST_CAMERA_PERMISSION)).setNegativeButton(android.R.string.cancel, (dialog, which) -> {
    ...
    private static class ImageSaver implements Runnable {
        ...
        ImageSaver(Image image, File file) { mImage = image; mFile = file; }
        @Override
        public void run() {
            ByteBuffer buffer = mImage.getPlanes()[0].getBuffer();
            byte[] bytes = new byte[buffer.remaining()];
            buffer.get(bytes);
            try (FileOutputStream output = new FileOutputStream(mFile)) { output.write(bytes); }
            catch (IOException e) { e.printStackTrace(); }
            finally { mImage.close(); }
        }
    }
}
